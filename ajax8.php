...
<?php while($res=mysqli_fetch_assoc($nn)){?>
    <tr>
        <?php $w=  mysqli_query($con, "SELECT COUNT( DISTINCT atd ) as y  FROM attendance WHERE date BETWEEN  '".$_GET['date1']."' AND  '".$_GET['date2']."' AND emp_id='".$res['employee_id']."' AND atd='P'");
